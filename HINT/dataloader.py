...
class Trial_Dataset(data.Dataset):
	def __init__(self, nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst, additional_feature_lst):
		self.nctid_lst = nctid_lst 
		self.label_lst = label_lst 
		self.smiles_lst = smiles_lst 
		self.icdcode_lst = icdcode_lst 
		self.criteria_lst = criteria_lst 
		self.additional_feature_lst = additional_feature_lst
	...
	def __getitem__(self, index):
		return self.nctid_lst[index], self.label_lst[index], self.smiles_lst[index], \
			   self.icdcode_lst[index], self.criteria_lst[index], self.additional_feature_lst[index] 
...
class Trial_Dataset_Complete(Trial_Dataset):
	def __init__(self, nctid_lst, status_lst, why_stop_lst, label_lst, phase_lst, 
					   diseases_lst, icdcode_lst, drugs_lst, smiles_lst, criteria_lst, additional_feature_lst):
		Trial_Dataset.__init__(self, nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst, additional_feature_lst)
	...
	def __getitem__(self, index):
		return self.nctid_lst[index], self.status_lst[index], self.why_stop_lst[index], self.label_lst[index], self.phase_lst[index], \
			   ...
			   self.additional_feature_lst[index] 
...
def trial_collate_fn(x):
	...
	additional_feature_lst = [i[5] for i in x]
	return [nctid_lst, label_vec, smiles_lst, icdcode_lst, criteria_lst, additional_feature_lst]
...
def csv_three_feature_2_dataloader(csvfile, shuffle, batch_size):
	...
	## nctid,status,why_stop,label,phase,diseases,icdcodes,drugs,smiless,criteria,additional_feature
	...
	additional_feature_lst = [torch.FloatTensor([float(i) for i in row[10].split('_')]).view(1,-1) for row in rows] 
	dataset = Trial_Dataset(nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst, additional_feature_lst)
