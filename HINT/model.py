...
class Interaction(nn.Sequential):
	...
	def generate_predict(self, dataloader):
		...
		for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, additional_feature_lst in dataloader:
			nctid_all.extend(nctid_lst)
			label_vec = label_vec.to(self.device)
			output = self.forward(smiles_lst2, icdcode_lst3, criteria_lst, additional_feature_lst).view(-1)  
	...
	def learn(self, train_loader, valid_loader, test_loader):
		...
		for ep in tqdm(range(self.epoch)):
			for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, additional_feature_lst in train_loader:
				label_vec = label_vec.to(self.device)
				output = self.forward(smiles_lst2, icdcode_lst3, criteria_lst, additional_feature_lst).view(-1)  #### 32, 1 -> 32, ||  label_vec 32,
				loss = self.loss(output, label_vec.float())
...
class HINT_nograph(Interaction):
	def __init__(self, molecule_encoder, disease_encoder, protocol_encoder, device, 
		...
		self.additional_feature_nn = nn.Linear(8,1)
	...
	def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, additional_feature_lst, if_gnn = False):
		...
		additional_feature_lst = torch.cat(additional_feature_lst, 0)
		additional_bias = self.additional_feature_nn(additional_feature_lst)
		output += additional_bias 
		if if_gnn == False:
			return output 
		else:
...
nn.ModuleList([ nn.ModuleList([nn.Linear(3,2) for j in range(5)] + [None]) for i in range(3)])
	...
	def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, 
					  additional_feature_lst, return_attention_matrix = False):
		embedding_lst = HINT_nograph.forward(self, smiles_lst2, icdcode_lst3, criteria_lst, 
												   additional_feature_lst, if_gnn = True)
